...
const ProductCard = ({ product }) => { // Receive the whole product object
  const { image, name, price, description, stock, _id } = product; // Destructure the product

  return (
    <div className="relative shadow-lg bg-stone-100">
      <div className="xl:flex lg:flex md:flex flex md:h-96 ">
        <div className="xl:w-1/2 lg:w-1/2 md:w-1/2 sm:w-1/2 w-1/2">
        ...
        <div className="lg:w-1/2 xl:w-1/2 md:w-1/2 w-1/2 p-6 justify-center object-cover">
          ...
          {/* Pass the whole product object to AddToCartBtn */}
    ...
    <>
      ...
      <div className="container mx-auto grid grid-cols-1 gap-5 lg:grid-cols-2 xl:px-20 lg:px-6 md:p-10 px-4 py-8">
        {filteredProducts.map((product) => (
          <ProductCard
            key={product._id}
            product={product} // Pass the whole product to ProductCard
          />
        ))}
      </div>
